...
function printOrderSlip(order){
  ...
  const logoUrl = `${location.origin}/nexaro-logo-compact-v2.jpg?v=30.1.85`;
...
async function loadShopConfigs(){
  ...
  Object.entries(templateDemos).forEach(([id,demo])=>{
    ...
    if(cfg) shopConfigs.set(id,{...cfg,...demo,customerId:id,logoFile:"/dein-logo.svg?v=30.1.85",logoHeight:90,active:true});
  });
  renderShopList();
