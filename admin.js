...
function printOrderSlip(order){
  ...
  const rows = (Array.isArray(order.items) ? order.items : []).map((item,index)=>{
    ...
    return `<tr><td>${index+1}</td><td>${htmlEscape(product)}</td><td>${htmlEscape(item.size)}</td><td>${htmlEscape(item.shirtColor)}</td><td>${htmlEscape(item.motif)}</td><td>${htmlEscape(item.motifColor)}</td><td>${qty}</td><td>${htmlEscape(euro(linePrice))}</td></tr>`;
  }).join("");
  ...
  <div class="section"><h2>Kundendaten</h2><div class="grid"><div class="field"><span>Vor- und Nachname</span><strong>${htmlEscape(order.name||"-")}</strong></div><div class="field"><span>Verein / Firma</span><strong>${htmlEscape(order.customerClass||"-")}</strong></div><div class="field"><span>E-Mail</span><strong>${htmlEscape(order.email||"-")}</strong></div><div class="field"><span>WhatsApp</span><strong>${htmlEscape(order.whatsapp||order.phone||"-")}</strong></div><div class="field"><span>Adresse</span><strong>${htmlEscape(order.address||"-")}</strong></div></div></div>
  <div class="section"><h2>Bestellung</h2><table><thead><tr><th>#</th><th>Textil</th><th>Größe</th><th>Farbe</th><th>Motiv</th><th>Motivfarbe</th><th>Menge</th><th>Preis</th></tr></thead><tbody>${rows}</tbody></table><div class="total"><span>${htmlEscape(order.totalQuantity||0)} Artikel</span><span>${htmlEscape(euro(order.totalPrice))}</span></div></div>
...
function renderOrder(id,order){
  ...
  [["Vor- und Nachname",order.name],["Verein / Firma",order.customerClass],["E-Mail",order.email],["WhatsApp",order.whatsapp||order.phone],["Adresse",order.address]].forEach(([label,value])=>{const box=document.createElement("div");const l=document.createElement("span");l.textContent=label;const v=document.createElement("strong");v.textContent=text(value);box.append(l,v);customer.appendChild(box)});
...
function buildShopConfig(){
  ...
  cfg.productPrint={
    ...
    hoodie:{front:{xPct:clamp(shopFields.hoodieFrontX.value,10,90,68),yPct:clamp(shopFields.hoodieFrontY.value,10,70,22),widthPct:clamp(shopFields.hoodieFrontW.value,8,70,36)},back:{xPct:clamp(shopFields.hoodieBackX.value,10,90,50),yPct:clamp(shopFields.hoodieBackY.value,10,70,34),widthPct:clamp(shopFields.hoodieBackW.value,10,90,78)}}
  };
  cfg.printData=collectPrintData();
  cfg.productionFile=(productionFileUrl?.value||"").trim();
  if(id === "tg-solingen") {
    cfg.hoodieSizingVersion = 5;
